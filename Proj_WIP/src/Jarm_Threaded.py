...
def send_packet(packet, destination_host, destination_port, Proxy):
    ...
    # print(destination_host, destination_port, Proxy)
    ...
    #Timeout errors result in an empty hash
        ...
        # print(f"An unexpected error occurred: {e}", destination_host)
        sock.close()
        return None
...
def jarm(destination_host, destination_port, Proxy):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(Serv_Questions)) as Exec:
        Results = Exec.map(lambda Serv_Questions: send_packet(packet_building(Serv_Questions), destination_host, destination_port, Proxy), Serv_Questions)
